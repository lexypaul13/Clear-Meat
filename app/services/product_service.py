...
def fetch_meat_products(db: Session):
    ...
    for meat_type in meat_types:
        ...
        # Fetch products for this meat type using the new API
        try:
            # Use text search with the meat type
            search_result = api.product.text_search(meat_type, page_size=100)
            ...
            if search_result and 'products' in search_result:
                ...
                for product_data in products:
                    try:
                        ...
                        ingredient_text = ' '.join([i.get('text', '') for i in ingredients if isinstance(i, dict) and 'text' in i])
            ...
            logger.error(f"Error fetching {meat_type} products: {str(e)}")
